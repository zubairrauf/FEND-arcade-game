...
let speedFactor = 25; //Used to increase the speed from level 6

/****Enemies 1: Cars from left to right*******/
var Enemy1 = function(x, y) {
    let randomCar = Math.floor(Math.random() * 6 + 1);
    let randomSpeed = Math.floor(Math.random() * 15 + 2);
...
var allEnemies = [enemy1, enemy2, enemy3, enemy4]; //Starting with 4 enemies
...
var Gem = function(x, y) {
    let randomGem = Math.floor(Math.random() * 5 + 1);
    this.sprite = 'images/Gem' +randomGem+'.png';
    this.x = x;
    this.y = y;
    this.update = function(dt) {  }
    };
...
//After three collision, trigger the lost function 
function lost(){
